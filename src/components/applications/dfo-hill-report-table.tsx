...
interface DfoHillReportTableProps {
  initialData: PaginatedApplications | null;
  accessToken: string;
  statuses: ApplicationStatusOption[];
}

export function DfoHillReportTable({ initialData, accessToken, statuses }: DfoHillReportTableProps) {
  ...
  const refreshData = useCallback(async () => {
    ...
    addLog(log || "Log for getApplications refresh in DFO Change of land use");
  ...
  const loadMoreApplications = useCallback(async () => {
    ...
    addLog(log || "Log for getApplications in DFO Change of land use");
  ...
  const handleRowClick = (app: any) => {
    router.push(`/dashboard/application/${app.id}?from=/dashboard/dfo-hill-report&workflow_sequence_id=${app.workflow_sequence_id}`);
  };
                  ...
                  <TableCell className="text-right">
                     <div className='flex justify-end items-center gap-2' onClick={(e) => e.stopPropagation()}>
                        <Button variant="outline" size="sm" asChild>
                            <Link href={`/dashboard/application/${app.id}?from=/dashboard/dfo-hill-report&workflow_sequence_id=${app.workflow_sequence_id}`}>View</Link>
                        </Button>
                    </div>
                  </TableCell>
                </TableRow>
