...
interface DfoReportTableProps {
  initialData: PaginatedApplications | null;
  accessToken: string;
  statuses: ApplicationStatusOption[];
}

export function DfoReportTable({ initialData, accessToken, statuses }: DfoReportTableProps) {
  ...
  const loadMoreApplications = useCallback(async () => {
    ...
    const { data: newData, log } = await getApplications(accessToken, nextPage, 10, 26);
    addLog(log || "Log for getApplications in DFO Report");
  ...
  const handleRowClick = (app: ApplicationListItem) => {
    router.push(`/dashboard/application/${app.id}?from=/dashboard/dfo-report&workflow_sequence_id=${app.workflow_sequence_id}`);
  };
                  ...
                  <TableCell className="text-right">
                     <div className='flex justify-end items-center gap-2' onClick={(e) => e.stopPropagation()}>
                        <Button variant="outline" size="sm" asChild>
                            <Link href={`/dashboard/application/${app.id}?from=/dashboard/dfo-report&workflow_sequence_id=${app.workflow_sequence_id}`}>View</Link>
                        </Button>
                        {/* <UpdateStatusForm
