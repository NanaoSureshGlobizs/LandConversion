...
interface SlcTableProps {
  initialData: PaginatedApplications | null;
  accessToken: string;
  statuses: ApplicationStatusOption[];
}

export function SlcTable({ initialData, accessToken, statuses }: SlcTableProps) {
  ...
  const loadMoreApplications = useCallback(async () => {
    ...
    addLog(log || "Log for getApplications in SLC List");
  ...
  const handleRowClick = (app: ApplicationListItem) => {
    router.push(`/dashboard/application/${app.id}?from=/dashboard/slc&workflow_sequence_id=${app.workflow_sequence_id}`);
  };
  ...
  const handleForward = async () => {
    ...
    addLog(`Forwarding applications with IDs: [${selectedIds.join(', ')}] from SLC List`);
                  ...
                  <TableCell className="text-right">
                     <div className='flex justify-end items-center gap-2' onClick={(e) => e.stopPropagation()}>
                        <Button variant="outline" size="sm" asChild>
                            <Link href={`/dashboard/application/${app.id}?from=/dashboard/slc&workflow_sequence_id=${app.workflow_sequence_id}`}>View</Link>
                        </Button>
                    </div>
                  </TableCell>
                </TableRow>
