...
const WORKFLOW_ID = 68;

interface LrdHillReportTableProps {
  initialData: PaginatedApplications | null;
  accessToken: string;
  statuses: ApplicationStatusOption[];
}

export function LrdHillReportTable({ initialData, accessToken, statuses }: LrdHillReportTableProps) {
  ...
  const refreshData = useCallback(async () => {
    ...
    addLog(log || "Log for getApplications refresh in LRD Change of land use");
  ...
  const loadMoreApplications = useCallback(async () => {
    ...
    addLog(log || "Log for getApplications in LRD Change of land use");
                  ...
                  <TableCell className="text-right">
                     <div className='flex justify-end items-center gap-2' onClick={(e) => e.stopPropagation()}>
                        <Button variant="outline" size="sm" asChild>
                            <Link href={`/dashboard/application/${app.id}?from=/dashboard/lrd-hill-report&workflow_sequence_id=${app.workflow_sequence_id}`}>View</Link>
                        </Button>
                    </div>
                  </TableCell>
                </TableRow>
