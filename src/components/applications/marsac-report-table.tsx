...
interface MarsacReportTableProps {
  initialData: PaginatedApplications | null;
  accessToken: string;
  statuses: ApplicationStatusOption[];
}

export function MarsacReportTable({ initialData, accessToken, statuses }: MarsacReportTableProps) {
  ...
  const loadMoreApplications = useCallback(async () => {
    ...
    const { data: newData, log } = await getApplications(accessToken, nextPage, 10, 30);
  ...
  const handleRowClick = (app: ApplicationListItem) => {
    router.push(`/dashboard/application/${app.id}?from=/dashboard/marsac-report&workflow_sequence_id=${app.workflow_sequence_id}`);
  };
                  ...
                  <TableCell>
                     <div className='flex justify-end items-center gap-2' onClick={(e) => e.stopPropagation()}>
                        <Button variant="outline" size="sm" asChild>
                            <Link href={`/dashboard/application/${app.id}?from=/dashboard/marsac-report&workflow_sequence_id=${app.workflow_sequence_id}`}>View</Link>
                        </Button>
                        {/* <UpdateStatusForm
