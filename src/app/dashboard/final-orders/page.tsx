...
import { FinalOrdersTable } from '@/components/applications/final-orders-table';
...
export default async function FinalOrdersPage({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }) {
  const cookieStore = cookies();
  const accessToken = cookieStore.get('accessToken')?.value;
  const type = (searchParams.type || 'diversion') as keyof typeof WORKFLOW_MAP;
    ...
    <>
      ...
      <div className="flex-1 space-y-4 px-4 md:px-8">
        <div className="flex items-center justify-between space-y-2">
          <h1 className="text-3xl font-bold tracking-tight font-headline">Final Orders ({type === 'diversion' ? 'Diversion' : ''})</h1>
        </div>
        <FinalOrdersTable
