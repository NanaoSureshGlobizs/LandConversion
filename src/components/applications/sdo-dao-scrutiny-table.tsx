...
interface SdoDaoScrutinyTableProps {
  initialData: PaginatedApplications | null;
  accessToken: string;
  statuses: ApplicationStatusOption[];
}

export function SdoDaoScrutinyTable({ initialData, accessToken, statuses }: SdoDaoScrutinyTableProps) {
    ...
    const results = await Promise.all(
        selectedIds.map(id => {
            const payload = {
                ...
                remark: "Forwarded from SDO/DAO Scrutiny",
                attachment: "",
                status: 1, 
            };
  ...
  const handleRowClick = (appId: number) => {
    router.push(`/dashboard/application/${appId}?from=/dashboard/sdo-dao-scrutiny`);
  };
                  ...
                  <TableCell>
                    <div className='flex justify-end items-center gap-2' onClick={(e) => e.stopPropagation()}>
                        <Button variant="outline" size="sm" asChild>
                            <Link href={`/dashboard/application/${app.id}?from=/dashboard/sdo-dao-scrutiny`}>View</Link>
                        </Button>
                    </div>
                  </TableCell>
                </TableRow>
